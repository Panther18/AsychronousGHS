...
public class Process implements Runnable {
	...
	boolean bBroadcastigNewLeader, bReceived2Broadcasts;
	boolean bTerminate;
	...
	Process(int id, int iRowToFind,float nbrs[][]){
		...
		bBroadcastigNewLeader=false;
		bReceived2Broadcasts=false;
		bTerminate=false;
	...
	public void send3NeighboursToFindMWOE(boolean bShouildITerminate){
		...
		objMessage.bShouldIQuit=bShouildITerminate;
	...
	public void inAbsorb_ConnectingProcess(){
	...
	public void inAbsorb_NonConnectingProcess(){
		addPendingNeighbor();
	...
	public void inMerge_Leader(int iMinID, int iMaxID){
		//I'm the leader
		...
		if(bShouldIConnect==true){
			...
			if(objRequestForConnect.connectionStatus.equals("absorb")==true){
				...
				if(objRequestForConnect.connectionStatus.equals("absorb")==true){
					...
					//		Decide who is the leader
					int iMinID=findMin_MaxID((int)objRequestForConnect.iMessageComponentID[1], (int)objRequestForConnect.iMessageComponentID[2], "min");
					int iMaxID=findMin_MaxID((int)objRequestForConnect.iMessageComponentID[1], (int)objRequestForConnect.iMessageComponentID[2], "max");
					if(iMyPID==iMinID){
						inMerge_Leader(iMinID, iMaxID);
					}
					else{
						inMerge_NonLeader();
					}
			...
			inAbsorb_NonConnectingProcess();
		}
	}

	public int findMin_MaxID(int iID1, int iID2, String sMinOrMax){
		int iMinID=iID1, iMaxID;
		if(iMinID>iID2){
			iMaxID=iMinID;
			iMinID=iID2;
		}
		...
		case "max":
			return iMaxID;
		default:
			System.out.println("Error no match found for either min/max");
			return 0;
		}
	}
	...
	public void receiveNewLeader_Broadcast(Message objMessage) {
		...
		if(bBroadcastigNewLeader=true){
			bReceived2Broadcasts = true;
	...
	public synchronized void scheduleMessage(Message objMessage){
		...
		int iRandomNumber, iProcessIndex=0, iCount;
		Long iSetTime;
